...
export class DashboardComponent implements OnInit {
  ...
  saveActivity(activity: Activity): void {
    ...
    this.configService.activities.push(activity);
  ...
  getTodoList() {
    this.todoList = this.configService.activities.filter(activity => !this.hasStartDate(activity));
  }

  getTodayList() {
    this.todayList = this.configService.activities.filter(activity => this.startDateIs(activity.startDate, 'today'));
  }

  getTomorrowList() {
    this.tomorrowList = this.configService.activities.filter(activity => this.startDateIs(activity.startDate, 'tomorrow'));
  }

  getDayAfterTomorrowList() {
    this.dayAfterTomorrowList = this.configService.activities.filter(activity => this.startDateIs(activity.startDate, 'dayAfterTomorrow'));
  }

  countActivitiesWithNoStartDate(): number {
    let filteredActivities = this.configService.activities.filter(activity => !this.hasStartDate(activity));
    this.todoList = filteredActivities;
    return filteredActivities.length;
  }

  countActivitiesForToday(): number {
    let filteredActivities = this.configService.activities.filter(activity => this.startDateIs(activity.startDate, 'today'));
    this.todayList = filteredActivities;
    return filteredActivities.length;
  }

  countActivitiesForTomorrow(): number {
    let filteredActivities = this.configService.activities.filter(activity => this.startDateIs(activity.startDate, 'tomorrow'));
    this.tomorrowList = filteredActivities;
    return filteredActivities.length;
  }

  countActivitiesForDayAfterTomorrow(): number {
    let filteredActivities = this.configService.activities.filter(activity => this.startDateIs(activity.startDate, 'dayAfterTomorrow'));
  ...
  onDrop(event: DragEvent, column: string) {
    ...
    const activity = this.configService.activities.find((a) => a.activityId === activityId);

    if (activity) {
      if (column === 'today') {
        activity.startDate = new Date().toISOString();
        activity.endDate = new Date().toISOString();
        let index = this.configService.activities.findIndex((a) => a.activityId === activity.activityId);
        if (index >= 0) {
          this.configService.activities[index].startDate = new Date().toISOString();
          this.configService.activities[index].endDate = new Date().toISOString();
        }
        ...
        if (index >= 0) {
          this.configService.activities[index].startDate = aux.toISOString();
          this.configService.activities[index].endDate = aux.toISOString();
        }
